...
from app.services.resource_service import fetch_contents_by_ids, fetch_account_by_id, fetch_content_groups_by_ids, \
    ...
    fetch_async_professional_contents_by_job_id, fetch_social_profiles_search_export_by_job_id
...
async def process_webhook(webhook_request_data: WebhookRequestData):
    if webhook_request_data.event == WebhookEvent.ACCOUNTS_CONNECTED \
        ...
        if is_professional_platform(webhook_request_data.work_platform.id):
        ...
        await process_social_profiles_search_export_event(webhook_request_data=webhook_request_data)
...
async def send_events(webhook_event: WebhookEvent, data: Dict, category: Optional[PlatformCategory] = None):
    for executor_event in EventExecutorRegistry.get_all_events():
        ...
        elif webhook_event == WebhookEvent.PROFILE_ANALYTICS_SUCCESS:
...
async def process_professional_contents_fetch_event(webhook_request_data: WebhookRequestData):
    ...
    professional_contents: Dict = await fetch_async_professional_contents_by_job_id(job_id=event.job_id)
...
async def process_social_profiles_search_export_event(webhook_request_data: WebhookRequestData):
    ...
    profiles_search_export: Dict = await fetch_social_profiles_search_export_by_job_id(job_id=profiles_search_export_event_job_id)
